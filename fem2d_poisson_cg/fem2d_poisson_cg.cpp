...
int *diag_index ( int m, int ia[], int ja[], int n );
...
double *solve_cg ( int node_num, int diag[], int nz_num, int ia[], int ja[], 
  double a[], double f[] );
...
//
  ...
  {
    cout << "\n";
    cout << "FEM2D_POISSON_CG - Fatal error!\n";
...
//
  ...
  if ( debug )
  {
    i4vec2_print ( nz_num, ia, ja, "  Adjacency pairs:" );
  }
...
//
  diag = diag_index ( nz_num, ia, ja, node_num );

  if ( debug )
  {
    i4vec_print ( node_num, diag, "  Diagonal adjacency vector:" );
  }
//
...
//
  node_u = solve_cg ( node_num, diag, nz_num, ia, ja, a, f );
...
//
{
  ...
  for ( j = 0; j < m; j++ )
  {
    if ( ia[j] == ja[j] )
    {
      ...
      {
        cerr << "\n";
        cerr << "DIAG_INDEX - Fatal error!\n";
        cerr << "  Multiple occurrences of diagonal pairs.\n";
        cerr << "  IA[" << j       << "] = JA[" << j       << "] = " << ia[j] << " and\n";
        cerr << "  IA[" << diag[i] << "] = JA[" << diag[i] << "] = " << ia[j] << "\n";
        exit ( 1 );
      }
      diag[i] = j;
    }
...
double *solve_cg ( int n, int diag[], int nz_num, int ia[], int ja[], 
  double a[], double b[] )
