#!/bin/bash
#
g++ -c -I/usr/local/dislin dislin_ex08.cpp >& compiler.txt
if [ $? -ne 0 ]; then
  echo "Errors compiling dislin_ex08.cpp"
  exit
fi
rm compiler.txt
#
g++ dislin_ex08.o -L/usr/local/dislin -ldislin -L/opt/local/lib -lXm -lm
if [ $? -ne 0 ]; then
