...
void xyzl_header_read ( string input_filename, int *line_num, int *line_data_num );
//
//  Global data.
//
  static GLint axis = 2;
...
//  Discussion:
//
//    This program reads XYZL information defining 3D points and lines,
...
//
{
  ...
  cout << "  to display the points and lines defined by an XYZL file.\n";
//
...
//
  if ( argc <= 1 ) 
  {
    cout << "\n";
    cout << "XYZL_DISPLAY_OPENGL:\n";
    cout << "  Please enter the \"prefix\" of the XYZ and XYZL files.\n";
...
//
//  Deal with the XYZL data.
//
  xyzl_filename = prefix + ".xyzl";

  xyzl_header_read ( xyzl_filename, &line_num, &line_data_num );

  cout << "\n";
  cout << "  The number of lines LINE_NUM           = " << line_num << "\n";
  cout << "  The number of line items LINE_DATA_NUM = " << line_data_num << "\n";

  line_pointer = new int[line_num+1];
  line_data = new int[line_data_num];

  xyzl_data_read ( xyzl_filename, line_num, line_data_num, line_pointer, line_data );

  if ( false )
  {
    xyzl_data_print ( point_num, line_num, line_data_num, line_pointer, line_data );
  }
//
//  If the XYZL data was created using 1-based indexing, we need to correct that,
//  if possible.
//
