...
void jacobian_adjust_dirichlet ( int node_num, double node_xy[],
  int node_p_variable[], int node_u_variable[], int node_v_variable[],
  int node_p_condition[], int node_u_condition[], int node_v_condition[],
  int variable_num, int ib, double a[] );
void jacobian_fem ( int node_num, double node_xy[], int element_num,
  int element_node[], int quad_num, int node_u_variable[],
  int node_v_variable[], int node_p_variable[], int variable_num,
  double nu, double c[], int ib, double a[] );
...
void reference_to_physical_t6 ( double t[2*6], int n, double ref[],
  double phy[] );
void residual_adjust_dirichlet ( int node_num, double node_xy[],
  int node_p_variable[], int node_u_variable[], int node_v_variable[],
  int node_p_condition[], int node_u_condition[], int node_v_condition[],
  int variable_num, double node_c[], double node_r[] );
void residual_fem ( int node_num, double node_xy[], int element_num,
  int element_node[], int quad_num, int node_u_variable[],
  int node_v_variable[], int node_p_variable[], int variable_num,
  double nu, double c[], double r[] );
...
//  Purpose:
//
//    MAIN is the main program for FEM2D_NAVIER_STOKES.
//
//  Discussion:
//
//    FEM2D_NAVIER_STOKES solves the steady incompressible Navier Stokes
//    equations for velocity vector W and scalar pressure P:
//
...
//
//    * the right hand side of anv Dirichlet boundary conditions:
//
...
//  Usage:
//
//    fem2d_navier_stokes nodes_file element_file
//
//    invokes the program:
//
...
//
//    Command line argument, char *NODE_FILE_NAME, the name of the
//    node file.  If this argument is not supplied, it will be requested.
//
//    Command line argument, char *ELEMENT_FILE_NAME, the name of
//    the element file.  If this argument is not supplied, it will be requested.
//
//  Local parameters:
//
...
//
//    Local, int IT_MAX, the maximum number of Newton iterations allowed.
...
//
//    Local, double NODE_C[VARIABLE_NUM], the finite element
//    coefficients of the current solution estimate.
//
//    Local, double NODE_C_DEL[VARIABLE_NUM], the correction to
//    the finite element coefficients of the current solution estimate.
//
//    Local, double NODE_C_OLD[VARIABLE_NUM], the finite element
...
//
{
  ...
  timestamp ( );
  cout << "\n";
  cout << "FEM2D_NAVIER_STOKES\n";
  cout << "  C++ version:\n";
  cout << "\n";
  cout << "  Compiled on " << __DATE__ << " at " << __TIME__ << ".\n";
  cout << "\n";
  cout << "  Finite element solution of the \n";
  cout << "  steady incompressible Navier Stokes equations\n";
  cout << "  on a triangulated region in 2 dimensions.'\n";
  cout << "\n";
  cout << "  - nu * ( Uxx + Uyy ) + UUx + VUy + dPdx = F1(x,y)\n";
  cout << "  - nu * ( Vxx + Vyy ) + UVx + VVy + dPdy = F2(x,y)\n";
  cout << "                                  Ux + Vy = F3(x,y).\n";
  cout << "\n";
  ...
  cout << "\n";
  cout << "  Maximum number of Newton iterations IT_MAX = " << it_max << "\n";
...
//
  ...
  {
    cout << "\n";
    cout << "FEM2D_NAVIER_STOKES - Fatal error!\n";
...
//
  ...
  node_c_del = new double[variable_num];
  node_c_old = new double[variable_num];
...
//
  ...
  {
    cout << "\n";
    cout << "  Solution to the STOKES Equations:\n";
    cout << "\n";
    cout << "      Node               U               V               P\n";
    cout << "\n";
...
//
  if ( false )
  {
    strcpy ( file_name, "stokes_pressure3.txt" );

    pressure3_write ( file_name, node_num, node_p_variable,
      variable_num, node_c );

    cout << "\n";
    cout << "  Stokes pressures written to \"" << file_name << "\".\n";
...
//
  ...
  for ( ; ; )
  {
    residual_fem ( node_num, node_xy, element_num,
    ...
    {
      node_r_norm = node_r_norm + pow ( node_r[variable], 2 );
    }
    node_r_norm = sqrt ( node_r_norm );

    cout << "\n";
    cout << "  l2-norm of FEM residual = " << node_r_norm << "\n";

    residual_adjust_dirichlet ( node_num, node_xy, node_p_variable,
      node_u_variable, node_v_variable, node_p_condition,
    ...
    cout << "\n";
    cout << "  l2-norm of adjusted FEM residual = " << node_r_norm << "\n";
    ...
    {
      cout << "\n";
      cout << "  The iteration limit has been reached.\n";
      break;
    }
...
//
    jacobian_fem ( node_num, node_xy, element_num,
      element_node, quad_num, node_u_variable,
...
//
    jacobian_adjust_dirichlet ( node_num, node_xy,
...
//
    ...
    node_c_del = dgb_sl ( variable_num, ib, ib, a, pivot, node_r, job );

    for ( variable = 0; variable < variable_num; variable++ )
    {
      node_c_old[variable] = node_c[variable];
    }
    for ( variable = 0; variable < variable_num; variable++ )
    {
      node_c[variable] = node_c[variable] - node_c_del[variable];
    }
    ...
    for ( variable = 0; variable < variable_num; variable++ )
    {
      node_c_del_norm = node_c_del_norm + pow ( node_c_del[variable], 2 );
    }
    node_c_del_norm = sqrt ( node_c_del_norm );

    cout << "\n";
    cout << "  l2-norm of Newton correction = " << node_c_del_norm << "\n";
  }
//
//  Print the Navier Stokes solution vector based at nodes.
//
  cout << "\n";
  cout << "  Solution to the NAVIER STOKES Equations:\n";
  cout << "\n";
  cout << "      Node               U               V               P\n";
  cout << "\n";
...
//
  ...
  cout << "\n";
  cout << "  Navier Stokes pressures written to \"" << file_name << "\".\n";
//
//  Write the velocities to a file.
//
  ...
  cout << "\n";
  cout << "  Navier Stokes velocities written to \"" << file_name << "\".\n";
//
//  Deallocate memory.
//
  delete [] a;
...
//    VARIABLE_NUM coefficient matrix, stored in a banded format; on output,
...
//    Input, double NU, the kinematic viscosity.
//
//    Input, double C[VARIABLE_NUM], the finite element
//    coefficients of an approximate solution of the Navier Stokes equations.
//
//    Input, int IB, the bandwidth of the jacobian.
//
//    Output, double A[(3*IB+1)*VARIABLE_NUM], the VARIABLE_NUM by
//    VARIABLE_NUM Navier Stokes jacobian, stored in a banded format.
//
{
  double aij;
  double area;
  ...
  dbdx = new double[6*quad_num];
  dbdy = new double[6*quad_num];
  dpdx = new double[3*quad_num];
  dpdy = new double[3*quad_num];
  dqdx = new double[3*quad_num];
  dqdy = new double[3*quad_num];
  dudx = new double[quad_num];
  dudy = new double[quad_num];
  dvdx = new double[quad_num];
  dvdy = new double[quad_num];
  ip = new int[3];
  iu = new int[6];
  iv = new int[6];
  p = new double[3*quad_num];
  q = new double[3*quad_num];
  quad_w = new double[quad_num];
  quad_xy = new double[2*quad_num];
  t3 = new double[2*3];
  t6 = new double[2*6];
  u = new double[quad_num];
  v = new double[quad_num];
  w = new double[quad_num];
  xy = new double[2*quad_num];
...
//
    for ( quad = 0; quad < quad_num; quad++ )
    {
      ...
      for ( j = 0; j < 6; j++ )
      {
         u[quad]   =  u[quad]   + cu[j] *  b[j+quad*6];
        dudx[quad] = dudx[quad] + cu[j] * dbdx[j+quad*6];
        dudy[quad] = dudy[quad] + cu[j] * dbdy[j+quad*6];
      }
      ...
      for ( j = 0; j < 6; j++ )
      {
         v[quad]   =  v[quad]   + cv[j] *  b[j+quad*6];
        dvdx[quad] = dvdx[quad] + cv[j] * dbdx[j+quad*6];
        dvdy[quad] = dvdy[quad] + cv[j] * dbdy[j+quad*6];
      }
      ...
      for ( j = 0; j < 3; j++ )
      {
         p[quad]   =  p[quad]   + cp[j] *  q[j+quad*3];
        dpdx[quad] = dpdx[quad] + cp[j] * dqdx[j+quad*3];
        dpdy[quad] = dpdy[quad] + cp[j] * dqdy[j+quad*3];
      }
    }
//
//  dUeqn/dUcof,
    ...
    for ( i = 0; i < 6; i++ )
    {
      for ( j = 0; j < 6; j++ )
      {
        ...
        for ( quad = 0; quad < quad_num; quad++ )
        {
          ...
          ( nu * ( dbdx[j+quad*6] * dbdx[i+quad*6]
                 + dbdy[j+quad*6] * dbdy[i+quad*6] )
          + ( b[j+quad*6] * dudx[quad]
            + u[quad] * dbdx[j+quad*6]
            + v[quad] * dbdy[j+quad*6] ) * b[i+quad*6]
          );
        }
        a[iu[i]-iu[j]+2*ib+(iu[j]-1)*bw]
      = a[iu[i]-iu[j]+2*ib+(iu[j]-1)*bw] + aij;

        aij = 0.0;
        for ( quad = 0; quad < quad_num; quad++ )
        {
          aij = aij + w[quad] * b[j+quad*6] * dudy[quad] * b[i+quad*6];
        }
        a[iu[i]-iv[j]+2*ib+(iv[j]-1)*bw]
      = a[iu[i]-iv[j]+2*ib+(iv[j]-1)*bw] + aij;
      }
      ...
      for ( j = 0; j < 3; j++ )
      {
        ...
        for ( quad = 0; quad < quad_num; quad++ )
        {
          aij = aij + w[quad] * dqdx[j+quad*3] * b[i+quad*6];
        }
        a[iu[i]-ip[j]+2*ib+(ip[j]-1)*bw]
      = a[iu[i]-ip[j]+2*ib+(ip[j]-1)*bw] + aij;
      }
    }
//
//  dVeqn/dUcof,
...
//
    for ( i = 0; i < 6; i++ )
    {
      for ( j = 0; j < 6; j++ )
      {
        ...
        for ( quad = 0; quad < quad_num; quad++ )
        {
          aij = aij + w[quad] * b[j+quad*6] * dvdx[quad] * b[i+quad*6];
        }
        a[iv[i]-iu[j]+2*ib+(iu[j]-1)*bw]
      = a[iv[i]-iu[j]+2*ib+(iu[j]-1)*bw] + aij;
        ...
        for ( quad = 0; quad < quad_num; quad++ )
        {
          ...
          ( nu * ( dbdx[j+quad*6] * dbdx[i+quad*6]
                 + dbdy[j+quad*6] * dbdy[i+quad*6] )
          + ( u[quad] * dbdx[j+quad*6]
            + b[j+quad*6] * dvdy[quad]
            + v[quad] * dbdy[j+quad*6] )
            * b[i+quad*6]
          );
        }
        a[iv[i]-iv[j]+2*ib+(iv[j]-1)*bw]
      = a[iv[i]-iv[j]+2*ib+(iv[j]-1)*bw] + aij;
      }
      ...
      for ( j = 0; j < 3; j++ )
      {
        ...
        for ( quad = 0; quad < quad_num; quad++ )
        {
          aij = aij + w[quad] * dqdy[j+quad*3] * b[i+quad*6];
        }
        a[iv[i]-ip[j]+2*ib+(ip[j]-1)*bw]
      = a[iv[i]-ip[j]+2*ib+(ip[j]-1)*bw] + aij;
      }
    }
//
//  dPeqn/dUcof,
...
//
    for ( i = 0; i < 3; i++ )
    {
      for ( j = 0; j < 6; j++ )
      {
        ...
        for ( quad = 0; quad < quad_num; quad++ )
        {
          aij = aij + w[quad] * dbdx[j+quad*6] * q[i+quad*3];
        }
        a[ip[i]-iu[j]+2*ib+(iu[j]-1)*bw]
      = a[ip[i]-iu[j]+2*ib+(iu[j]-1)*bw] + aij;

        aij = 0.0;
        for ( quad = 0; quad < quad_num; quad++ )
        {
          aij = aij + w[quad] * dbdy[j+quad*6] * q[i+quad*3];
        }
        a[ip[i]-iv[j]+2*ib+(iv[j]-1)*bw]
      = a[ip[i]-iv[j]+2*ib+(iv[j]-1)*bw] + aij;
      }
    }
  }
...
//    Input, int VARIABLE_NUM, the number of variables.
//
//    Input, double NODE_C[VARIABLE_NUM], the finite element
//    coefficient vector.
//
//    Input/output, double NODE_R[VARIABLE_NUM], the
...
//
{
  ...
  for ( node = 0; node < node_num; node++ )
  {
    ...
    if ( node_u_condition[node] == DIRICHLET )
    {
      node_r[iu-1] = node_c[iu-1] - u_bc[node];
    }
    if ( node_v_condition[node] == DIRICHLET )
    {
      node_r[iv-1] = node_c[iv-1] - v_bc[node];
    }
    if ( 0 < ip )
    {
      if ( node_p_condition[node] == DIRICHLET )
      {
        node_r[ip-1] = node_c[ip-1] - p_bc[node];
      }
    }
  }
  delete [] u_bc;
...
//    coefficients of an approximate solution of the Navier Stokes equations.
//
//    Output, double R[VARIABLE_NUM], the Navier Stokes residual.
//
{
  double area;
  double *b;
...
//
    for ( i = 0; i < 6; i++ )
    {
      for ( quad = 0; quad < quad_num; quad++ )
      {
        r[iu[i]-1] = r[iu[i]-1] + w[quad] *
        ( nu * ( dudx[quad] * dbdx[i+quad*6] + dudy[quad] * dbdy[i+quad*6] )
          + ( u[quad] * dudx[quad] + v[quad] * dudy[quad]
          + dpdx[quad] - u_rhs[quad] ) * b[i+quad*6]
        );
      }
    }
//
...
//
    for ( i = 0; i < 6; i++ )
    {
      for ( quad = 0; quad < quad_num; quad++ )
      {
        r[iv[i]-1] = r[iv[i]-1] + w[quad] *
        ( nu * ( dvdx[quad] * dbdx[i+quad*6] + dvdy[quad] * dbdy[i+quad*6] )
          + ( u[quad] * dvdx[quad] + v[quad] * dvdy[quad]
          + dpdy[quad] - v_rhs[quad] ) * b[i+quad*6]
        );
      }
    }
//
...
//
    for ( i = 0; i < 3; i++ )
    {
      for ( quad = 0; quad < quad_num; quad++ )
      {
        r[ip[i]-1] = r[ip[i]-1] + w[quad] *
        ( dudx[quad] + dvdy[quad] - p_rhs[quad] ) * q[i+quad*3];
      }
    }
...
//
//    Output, bool S_TO_IVEC, is TRUE if an error occurred.
//
{
  bool error;
  int i;
