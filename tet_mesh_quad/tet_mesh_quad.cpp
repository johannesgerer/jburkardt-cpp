...
//
{
  ...
  cout << "TET_MESH_QUAD:\n";
  cout << "  C++ version:\n";
  cout << "  Estimate an integral over a region defined by a tetrahedral mesh.\n";
  cout << "\n";
...
//
  ...
  r8mat_transpose_print_some ( node_dim, node_num, node_xyz, 1, 1, node_dim, 5, 
    "  First 5 nodes:" );
//
...
//
  ...
  cout << "\n";
  cout << "  Read the header of \"" << value_filename << "\".\n";
  cout << "\n";
  cout << "  VALUE dimension VALUE_DIM = " << value_dim << "\n";
  cout << "  Number of nodes VALUE_NUM = " << value_num << "\n";

  if ( value_num != node_num )
  {
    cerr << "\n";
    cerr << "TRIANGULATION_QUAD - Fatal error!\n";
    cerr << "  Number of sets of values must equal number of  nodes.\n";
    exit ( 1 );
  }
  value = r8mat_data_read ( value_filename, value_dim, value_num );

  cout << "\n";
  cout << "  Read the data in \"" << value_filename << "\".\n";
...
//
  ...
  {
    volume[element] = ( 1.0 / 6.0 ) * r8_abs (                                               
        ( node_xyz[0+element_node[1+element*element_order]*3] - node_xyz[0+element_node[0+element*element_order]*3] )       
        * ( ( node_xyz[1+element_node[2+element*element_order]*3] - node_xyz[1+element_node[0+element*element_order]*3] )   
          * ( node_xyz[2+element_node[3+element*element_order]*3] - node_xyz[2+element_node[0+element*element_order]*3] )   
          - ( node_xyz[2+element_node[2+element*element_order]*3] - node_xyz[2+element_node[0+element*element_order]*3] )   
          * ( node_xyz[1+element_node[3+element*element_order]*3] - node_xyz[1+element_node[0+element*element_order]*3] ) ) 
      + ( node_xyz[1+element_node[1+element*element_order]*3] - node_xyz[1+element_node[0+element*element_order]*3] )       
        * ( ( node_xyz[2+element_node[2+element*element_order]*3] - node_xyz[2+element_node[0+element*element_order]*3] )   
          * ( node_xyz[0+element_node[3+element*element_order]*3] - node_xyz[0+element_node[0+element*element_order]*3] )   
          - ( node_xyz[0+element_node[2+element*element_order]*3] - node_xyz[0+element_node[0+element*element_order]*3] )   
          * ( node_xyz[2+element_node[3+element*element_order]*3] - node_xyz[2+element_node[0+element*element_order]*3] ) ) 
      + ( node_xyz[2+element_node[1+element*element_order]*3] - node_xyz[2+element_node[0+element*element_order]*3] )       
        * ( ( node_xyz[0+element_node[2+element*element_order]*3] - node_xyz[0+element_node[0+element*element_order]*3] )   
          * ( node_xyz[1+element_node[3+element*element_order]*3] - node_xyz[1+element_node[0+element*element_order]*3] )   
          - ( node_xyz[1+element_node[2+element*element_order]*3] - node_xyz[1+element_node[0+element*element_order]*3] )   
          * ( node_xyz[0+element_node[3+element*element_order]*3] - node_xyz[0+element_node[0+element*element_order]*3] ) ) );
...
//
  ...
  cout << "\n";
  cout << "  Tet mesh volume = " << r8vec_sum ( element_num, volume ) << "\n";
